<template>
  <div>
    <PageHeader title="Reports" subtitle="Sales trends, traffic and conversion at a glance.">
      <template #actions>
        <select class="input w-full sm:w-auto sm:max-w-[180px]">
            ...
            <tr v-for="p in top" :key="p.id">
              <td>
                <div class="flex items-center gap-3">
                  <img :src="p.image" class="w-9 h-9 rounded-lg object-cover bg-cream shrink-0" />
                  <p class="font-medium truncate">{{ p.name }}</p>
                </div>
              </td>
              <td class="text-right whitespace-nowrap">{{ p.sales }}</td>
              <td class="text-right font-semibold whitespace-nowrap">{{ formatMoney(p.sales * p.price) }}</td>
